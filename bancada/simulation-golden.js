...
function campaignSeriesScenario(){
  ...
  assert.equal(result.mapas.map(map=>map.mapa).join(","),"Nuke,Dust2,Anubis",`${id}: sequencia de mapas mudou antes de atualizar o fixture`);
  return {
...
function buildCurrent(){
  const current={
    ...
    scenarios:[
      ...
      fixedMapScenario({id:"repeated-overtime",seed:8,aIndex:0,bIndex:1,map:"Nuke",expectedScore:[22,19]}),
