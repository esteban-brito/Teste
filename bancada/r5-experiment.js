/* bancada/r5-experiment.js - captura e compara observacoes pareadas do R5.
...
const {buildDeepAudit}=require("./auditoria");
...
function gameSha256(){
  return sha256(fs.readFileSync(path.join(__dirname,"..","game.js")));
}
...
async function pairedStatistics(){
  if(!pairedStatisticsPromise){
    const moduleUrl=pathToFileURL(path.join(__dirname,"..","src","domain","statistics","paired-comparison.mjs")).href;
...
async function main(args=process.argv.slice(2)){
  ...
  console.log("Uso:\n  node bancada/r5-experiment.js capture --cycles 8 --output <novo.json>\n  node bancada/r5-experiment.js compare --before <baseline.json> --after <candidate.json> [--output <novo.json>]");
}
