...
// função (primária) por nick — para agrupar A/K por papel
const roleByNick={};
...
for(let c=0;c<N;c++){
  ...
  for(const team of T){
    for(let m=0;m<MAPS;m++){
      ...
      const game=X.simularMapa(team,opp,X.forcaDoDia(team.ef,team.quim),X.forcaDoDia(opp.ef,opp.quim));
      acc(game.statsA,game.totalRounds);acc(game.statsB,game.totalRounds);
    }
  }
}
