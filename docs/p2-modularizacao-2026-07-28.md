...
| 5 | API pública + 6 consumidores Node | **em andamento** — 2 de 6 migrados |
...
calibráveis sem criar uma cópia divergente. `tools/verify-report.js` e o harness
`bancada/worker-calibrador.js` já consomem essa composição sem ler nem executar
um recorte de `game.js`. O worker real ainda é legado e pertence à Fase 6.

Restam quatro consumidores Node do recorte legado: `bancada/motor.js`,
`bancada/calibrador-loader.js`, `tools/check-engine-exports.js` e
`tools/check-sandbox-engine.js`. Migrá-los um por vez, sem retirar uma guarda antes
de seu contrato ter substituto explícito.
